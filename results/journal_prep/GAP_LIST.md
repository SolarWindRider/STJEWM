...
| CuBiFAE | 0.105 | 0.422 | 0 | 0.9988 | 0.476 | 0.518 | 0.497 | — | — | — | — | — | — | — | — |
| SLT-trace | 0.091 | 0.422 | 0 | 0.9996 | **0.671** | **0.668** | **0.675** | — | — | — | — | 0.115±0.004 | — | — | — |
| SLT-free | 0.105 | 0.386 | 0 | 0.9997 | **0.588** | **0.591** | **0.582** | — | — | — | — | — | — | — | — |
...
| SpikeDreamer | 0.000 | 1.000 | 0 | -0.0003 | **0.546** | **0.548** | **0.556** | — | — | — | — | — | — | — | — |

## GAPS to fill (experiment → missing models)

### G1. event-ρ — **DONE**
- Complete 13-model × 4-env × 2-split coverage: `G1_event_align_complete/summary.md`.
- Mean $\rho$: STJEWM-trace 0.9987; STJEWM-spike 0.9988; STJEWM-rate 0.9988; STJEWM-no-trace 0.9987; STJEWM-leak 0.9986; STJEWM-membrane 0.9987; CuBiFAE 0.9988; SLT-trace 0.9996; SLT-free 0.9997; LeWM-v2 0.7515; GRU -0.0074; MLP -0.0233; SpikeDreamer -0.0003.
- Verdict: STJEWM and SLT exceed 0.9, but the full “SNN > 0.9 vs non-SNN < 0.8” claim does not hold because CuBiFAE reaches 0.9988.
...
Key findings: SLT-trace 0.672 > LeWM-v2 0.626 > SLT-free 0.587 > GRU 0.546 > SpikeDreamer 0.543;
...
- Key verdict: LeWM position R² = +0.285 (avg over 10 envs); STJEWM readouts (6 readouts) avg position R² = −0.042; MLP/GRU/CubifAE/SLT-LIF-MPC/SpikeDreamer are also near chance. The dissociation is **not** STJEWM-specific — every recurrent / local-network controller is similarly opaque, while **only LeWM is highly decodable**.

### G5. 3-seed cos — **DONE (2026-08-04)**
- 47/48 ckpts trained + 47/48 evaluated across 3 splits × 8 new models × 2 seeds.
  (1 missing: `slt_lif_mpc_free` generalist_16env seed=2 — caught mid-training to
...
- Cluster verdict preserved at 3-seed resolution with FULL model coverage:
  - **collapse** (cos≈0): spikedreamer_baseline, mlp_baseline, **gru_baseline** (new).
  - **calibrated** (cos≈0.11–0.14, all CIs pairwise overlap): all 6 STJEWM readouts
    ...
    both SLT-LIF-MPC variants, and cubifae_baseline.
  - **over-react** (cos≈0.19): lewm_baseline_v2.
  - All 9 calibrated models vs LeWM: 95% CIs disjoint (Cohen's d ≤ −4.5).
  - All 9 calibrated models vs MLP/GRU/SpikeDreamer: 95% CIs disjoint (Cohen's d > 16).
