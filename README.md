...
### v0.7.10b — OOD path-C (3-family DMC cross-sub-family transfer)

The 3 DMC sub-families — F1 classic control (8 envs), F2 locomotion
(7 envs), F3 sparse-POMDP (11 envs) — are split into 6 train/held-out
splits: 3 OOD1 (1 family trained, 2 unseen) and 3 OOD2 (2 families trained,
1 unseen). 12 ckpts × 6 splits × held-out envs = 468 cells total,
all four collapse-robust metrics (divergence / responsiveness / ρ / env-SR)
populated.

The runner trains 12 ckpts per split (~25 min each on 1 CPU, ~30 min ×
72 ckpts total per seed) and writes per-cell JSONs under
`results/oodc/<split>/<split>/<model>/seed_0/`. Closed-loop env-SR uses
3 episodes per held-out env with a CEM planner (200 steps each).

```bash
# One-shot: train all 72 ckpts (6 splits × 12 models) — ~30 hr sequential on 1 CPU
# (use --skip-train and only --aggregate-only if ckpts are already on disk)
PYTHONPATH=/home/lx/snn python code/scripts/utility/ood1_path_c.py

# Just re-aggregate from existing per-cell JSONs (no retraining, ~1 sec)
PYTHONPATH=/home/lx/snn python code/scripts/utility/reaggregate_ood1.py
```

The aggregate table is at `results/utility/ood1_table.md` and reports
both per-cell, per-(split, model) and per-(split, family) means. Key
finding: STJEWM 6 readouts keep `div ∈ [0.0097, 0.1350]`, `resp ≈ 0.20`,
`ρ ∈ [0.9676, 0.9986]` across all 6 OOD splits; LeWM over-reacts
(`resp 2.4–6.2`), GRU under-fits (`resp ≈ 0.10`), MLP collapses
(`resp ≈ 0.0007`).

If cubifae / slt_lif_mpc baselines show `div = nan`, that means
`build_model_from_ckpt` is reading env-native `state_dim` instead of the
trained `pad_obs_to=128`. The fix (committed in v0.7.10b) is to pass
`pad_obs_to=128` and `action_dim=56` to the cubifae / slt builders,
mirroring how the ckpts were originally trained.

## Status (v0.7.10b, 2026-07-10)
...
| **v0.7.10b OOD path-C (3-family DMC cross-sub-family transfer)** | **done** | 6 splits × 12 ckpts × held-out envs = 468 cells, all four metrics populated (incl. cubifae after `build_model_from_ckpt` patch). STJEWM 6 readouts keep `ρ ∈ [0.9676, 0.9986]` across all 6 splits; LeWM over-reacts (`resp` 2.4–6.2), GRU under-fits (`resp ≈ 0.10`), MLP collapses (`resp ≈ 0.0007`). `results/utility/ood1_table.md` regenerated by `code/scripts/utility/reaggregate_ood1.py`. |
