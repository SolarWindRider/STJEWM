"""ST-JEWM v4 — Pure SNN world model (A2 + B1).

This is the response to the NMI reviewer challenge that v3 (LeWM Transformer
+ SNN side-channel) is a hybrid, not a pure SNN model. v4 **fully replaces**
the LeWM Transformer with a stack of MultiCompartment SNN cells.

A2: MultiCompStack — a 4-layer stack of MultiCompartment SNN cells with
    LayerNorm + residual connections. NO Transformer, NO AdaLN, NO attention.

B1: Gated Spike Trace (reused verbatim from v3) — content-aware exponential
    trace r_t = alpha_t * r_{t-1} + (1 - alpha_t) * s_t where
    alpha_t = sigma(W_gate * [r_{t-1}, s_t, c_t]).
...
Architecture (pixel input):
    ...
    z_final = h + trace_proj(r_t)

Architecture (state input, e.g. DMC):
    state (B, T, state_dim) -> state_proj -> z_enc (B, T, 192)
    action (B, T, action_dim) -> ActionMLP -> a_emb (B, T, 192)
    ... same as pixel pipeline.

Parameter budget (target ~5M total):
    ViT-Tiny (frozen):  ~5.4M (NOT counted as trainable)
    Projector:           ~0.4M
    ActionMLP:           ~0.04M
    MultiCompStack x4:   ~4.0M (4 * ~1.0M per cell)
    Gated Spike Trace:   ~0.15M
    trace_proj:          ~0.04M
    Total TRAINABLE:     ~4.6M
"""
...
class MultiCompStack(nn.Module):
    ...
    def __init__(
        ...
        # 12x expansion -> per cell ~885K params, stack ~3.5M, total v4 ~5M
...
class ActionMLP(nn.Module):
    ...
    LeWM's Embedder uses 1D-conv smoothing + 2-layer MLP, but for v4 we want
...
class STJEWMv4(nn.Module):
    ...
    def __init__(
        ...
        # we use a simpler 2-layer MLP for v4 to keep params low.
...
def make_stjewm_v4(
    ...
    """Factory: build a pure-SNN v4 world model.
...
if __name__ == "__main__":
    ...
    # Pixel-input model
    print("=" * 60)
    print("STJEWMv4 smoke test — pixel input")
    print("=" * 60)
    model = make_stjewm_v4(obs_dim=3 * 224 * 224, action_dim=10, n_layers=4, n_neurons=192)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    n_total = sum(p.numel() for p in model.parameters())
    n_frozen = n_total - n_params
    print(f"STJEWMv4 params: trainable={n_params:,}  total={n_total:,}  frozen={n_frozen:,}")
    ...
    # Test predict() API (v3-compatible)
    ...
    print()
    print("=" * 60)
    print("STJEWMv4 smoke test — state input (DMC)")
    print("=" * 60)
    model_state = make_stjewm_v4(obs_dim=9, action_dim=1, n_layers=4, n_neurons=192)
