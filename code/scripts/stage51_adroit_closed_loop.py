"""AdroitHand closed-loop sim eval (v5 world model).

AdroitHand has 4 envs (door, hammer, pen, relocate) with 24-30 DOF.
Uses AdroitHand-specific XML directory.
"""
import argparse
import json
import sys
...
XML_BASE = "/home/lx/miniconda3/envs/snn/lib/python3.10/site-packages/gymnasium_robotics/envs/assets/adroit_hand/"

ENVS = {
    'adroit_door':     ('adroit_door.xml',     28, 'door'),
    'adroit_hammer':   ('adroit_hammer.xml',   26, 'hammer'),
    'adroit_pen':      ('adroit_pen.xml',      24, 'pen'),
    'adroit_relocate': ('adroit_relocate.xml', 30, 'relocate'),
}
...
def main():
    args = parse_args()
    print(f"[adroit-closed-loop] env={args.env}, ckpt={args.ckpt}", flush=True)
    m = mujoco.MjModel.from_xml_path(XML_BASE + ENVS[args.env][0])
    state_dim = m.nq
    action_dim = ENVS[args.env][1]
