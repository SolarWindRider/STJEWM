"""Eval LeWM-style Transformer baseline on target-conditioned reach task."""
import argparse
import json
import sys
...
def load_model(ckpt_path, state_dim, action_dim):
    ...
    sd = ck.get("model", ck)
    model = LeWMTransformerBaseline(state_dim=state_dim, action_dim=action_dim)
...
def main():
    args = parse_args()
    print(f"[lewm-baseline-plan] ckpt={args.ckpt}", flush=True)
    ...
    for ep in range(args.n_episodes):
        ...
        success = final_dist_qpos < 0.1
        results.append({"ep": ep, "init_qpos_dist": init_dist_qpos, "final_qpos_dist": final_dist_qpos, "improved": improved, "success": success})
