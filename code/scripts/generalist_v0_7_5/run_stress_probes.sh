#!/bin/bash
# Run linear-probe event-AUROC on the 4 stress envs for every
# generalist (suite, model, env, target).
#
# Usage:
...
#
# extra_flags in the spec (split, goal-offset, flicker-mask-ratio,
# vel-hidden-mask-obs-ratio) configure the live closed_loop env that
# the planner runs in; they do NOT apply to the offline linear probe
# (which reads pre-collected trajectories from <path>), so we drop them.
#
# Output goes to results/<suite>/stress_probe/<env>_<model>_<target>.json
# AND mirrors to results/aggregate/event_probes/ for the aggregator.
set -e
cd /home/lx/snn
