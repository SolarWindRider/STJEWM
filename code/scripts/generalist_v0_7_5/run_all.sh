...
#
# Usage:
#   bash code/scripts/generalist_v0.7.5/run_all_v0.7.5.sh [--seeds N] [--skip-g4] [--skip-g8] [--skip-g16]
#
...
if [[ -z "$SKIP_G4" ]]; then
    echo ""
    echo "[run_all] === G4 ==="
    bash code/scripts/generalist_v0.7.5/run_suite.sh G4 \
...
if [[ -z "$SKIP_G8" ]]; then
    echo ""
    echo "[run_all] === G8 ==="
    bash code/scripts/generalist_v0.7.5/run_suite.sh G8 \
...
if [[ -z "$SKIP_G16" ]]; then
    echo ""
    echo "[run_all] === G16 ==="
    bash code/scripts/generalist_v0.7.5/run_suite.sh G16 \
...
for SUITE in G4 G8 G16; do
    /home/lx/miniconda3/envs/snn/bin/python -m code.scripts.generalist_v0.7.5.aggregate_master --suite "$SUITE"
done
