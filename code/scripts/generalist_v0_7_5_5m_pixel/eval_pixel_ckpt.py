...
def main():
    ...
    for env_kind in DMC_ENVS:
        ...
        try:
            ...
            goal_z = encode_obs(model, goal_pixel, action_dim_env, args.device).unsqueeze(0).unsqueeze(0)  # (1, 1, D)
            ...
            for ep in range(args.n_episodes):
                ...
                state = env.get_state()
                for t in range(200):
                    cur_z = encode_obs(model, obs["pixel"], action_dim_env, args.device)
                    cur_z = cur_z.unsqueeze(0).unsqueeze(0)  # (1, 1, D)
                    # CEM plan
                    with torch.no_grad():
                        action_seq = cem.plan(cur_z, goal_z)  # (H, A)
