...
XML = "/home/lx/miniconda3/envs/snn/lib/python3.10/site-packages/ogbench/manipspace/descriptions/universal_robots_ur5e/ur5e.xml"
    ...
    for ep in range(n_ep):
        ...
        for i in range(m.njnt):
            ...
            if r is not None and r[0] < r[1]:
                center = (r[0] + r[1]) / 2
                half = (r[1] - r[0]) / 2 * 0.3
                d.qpos[m.jnt_qposadr[i]] = rng.uniform(center - half, center + half)
        d.qvel[:] = 0
...
def main():
    ...
    print(f"[ur5e-cl] state={m.nq} act={m.nu} params={sum(p.numel() for p in model.parameters())/1e6:.2f}M", flush=True)
    ...
    with open(out_path, "w") as f:
        json.dump({"env": "ur5e", "ckpt": args.ckpt, "n_episodes": args.n_episodes, "ep_len": args.ep_len, "n_nan_eps": nans,
