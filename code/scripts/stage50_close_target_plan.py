"""v5 SNN target-conditioned planning with CLOSE goal (within 0.5 qpos distance).

This is a relaxed version of stage46: pick init and goal configs that are close in qpos space.
The model should be able to plan a short trajectory.
"""
import argparse
import json
import sys
...
def parse_args():
    ...
    p.add_argument("--max-init-dist", type=float, default=0.5, help="Max init-goal qpos distance")
...
def find_close_init_goal(m, d, rng, target_pos, max_dist=0.5):
    ...
    # Sample many random configs that put fingertip near target
    candidates = []
    for _ in range(500):
        for j in arm_joints:
            qs = m.jnt_qposadr[j]
        ...
        if ft_dist < 0.03:  # finger near target
            candidates.append(d.qpos[:].copy())
    ...
    # Find pair with smallest qpos distance
    best = None
    best_dist = float('inf')
    for i in range(min(10, len(candidates))):
        for j in range(i+1, min(10, len(candidates))):
            dist = float(np.linalg.norm(candidates[i] - candidates[j]))
...
def main():
    args = parse_args()
    print(f"[v5-close-reach] ckpt={args.ckpt}", flush=True)
    ...
    for ep in range(args.n_episodes):
        ...
        pair = find_close_init_goal(m, d, rng, target_pos, max_dist=args.max_init_dist)
        if pair is None:
            continue
        ...
        for j in range(m.njnt):
            ...
            qs = m.jnt_qposadr[j]
            jname = mujoco.mj_id2name(m, mujoco.mjtObj.mjOBJ_JOINT, j) or ''
            if jt == mujoco.mjtJoint.mjNT_FREE if False else (jt == mujoco.mjtJoint.mjJNT_FREE):
                if 'ball' in jname or 'peg' in jname:
                    d.qpos[qs:qs+7] = [0, 0, 0, 1, 0, 0, 0]
                else:
                    d.qpos[qs:qs+3] = [0, 0, 0]
        ...
        init_state = np.concatenate([d.qpos[:14], target_pos]).astype(np.float32)
        ...
        for step in range(args.max_steps):
            if step % args.replan_every == 0:
                cur_emb = encode_state(model, np.concatenate([d.qpos[:14], target_pos]).astype(np.float32), action_dim)
    ...
    final = {
        "env": "manipulator_close_target_plan",
