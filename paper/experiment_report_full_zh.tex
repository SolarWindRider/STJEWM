...
& \path{stjewm_membrane_readout} & $z=h$，违反协议（规划器可读膜电位） & 2.70M / 8.20M \\
\midrule
...
\textbf{参数量注（v0.7.5 原始设置）}：STJEWM 的 6 个 readout 共享同一骨架（仅 readout 不同），trainable=2.70M，total=8.20M（含冻结的 ViT-Tiny 编码器 5.50M）；SNN/Transformer 基线各自独立骨架；MLP 基线是"坍缩对照"——其设计目的就是验证"无记忆能力"会导致 div $\to 0$。
...
\begin{adjustbox}{max width=\textwidth}
\scriptsize
\begin{tabular}{p{0.16\textwidth}p{0.15\textwidth}p{0.06\textwidth}p{0.13\textwidth}p{0.13\textwidth}p{0.13\textwidth}p{0.13\textwidth}}
\toprule
...
\subsection{轴 2 (v0.7.5, 192 cells, 12 模型 × 4 splits) — 修正前的 33.7× 参数不公平衡量}

\textbf{本节先呈现 v0.7.5 原始数据，再于 §6.3 给出 v0.7.14 5M-aligned 修正后的结果。}

每个 (model, split) 的 \texttt{mean\_cos\_dist}（来自 \path{results/cross\_benchmark\_F\{1,2,3,4\}/eval/}）：

\begin{adjustbox}{max width=\textwidth}
\scriptsize
\begin{tabular}{p{0.22\textwidth}p{0.11\textwidth}p{0.11\textwidth}p{0.11\textwidth}p{0.11\textwidth}p{0.11\textwidth}}
\toprule
...
%==========================================================
\subsection{轴 2 (v0.7.14 5M-aligned, 130/130 ckpts) — 参数公平的 3 splits × 13 模型}

\textbf{修正动机}：v0.7.5 的 192-cell 对比中，STJEWM trainable=2.70M vs 其他基线 0.22-7.42M 范围（33.7× 差距）。这意味着 cos\_dist 的差异可能被参数规模混淆——\textbf{坍缩可能只是因为模型不够大}。v0.7.14 把所有 8 个基线重新训练到 4.97-5.13M（range 0.16M，$\pm 3.2\%$），STJEWM 保持 trainable=5.06M。现在 SOTA 对比是\textbf{参数公平的}。
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{跨基准族任务表现}（轴 2，192 cells v0.7.5 \textbf{+} 5M-aligned 390 cells v0.7.14）：在 5M 参数公平下，STJEWM 6 readouts 在 3 个 cross-benchmark split 上达到 mean 51-58\% LeWM-SR（$\cos\_dist \in [0.04, 0.20]$），与 \textbf{CubifAE}（56\%）和 \textbf{SLT-LIF-MPC-free}（55\%）处于同一 collapse-robust 簇，\textbf{trace dynamics 假设的物理边界在 5M 参数下仍然成立}。SLT-LIF-MPC-trace（72\%）在 F2/F3 上特别强，GRU（87\%）和 SpikeDreamer/MLP（95-100\%）是坍缩/过反应 artifact。\textbf{特定 readout winner 因 split 而异}，但 STJEWM 6 readouts 都赢 $\to$ 跨 readout 的鲁棒性是 STJEWM 的核心优势。
    \item \textbf{新增 (v0.7.14)：trace dynamics 假设对参数规模鲁棒}。5M-aligned 验证：当所有基线从 0.22-7.42M 范围重新训练到 4.97-5.13M（$\pm 3.2\%$）后，trace / spike / rate / no-trace / leak / membrane 六种 readout 都保持 resp $\approx 0.21$、div $\approx 0.006$、$\cos\_dist \in [0.04, 0.20]$，与 MLP（div=0）、LeWM-v2（resp $\approx 10$）的坍缩/过反应\\textbf{清晰分离}。\\textbf{trace dynamics 不是"参数堆出来的"}，而是 trace 自身的内容感知衰减的内禀属性。
...
\textbf{5M-aligned 关键超参}：与 v0.7.5 相同的 optimizer / loss / batch / seed，但 \texttt{--hidden-dim}、\texttt{--mlp-hidden}、\texttt{--mlp-layers}、\texttt{--slt-layers}、\texttt{--slt-din} 这 5 个新 CLI flag 在 \texttt{build\_model} 内部被自动设为各模型的 5M-aligned 默认值。训练时间从 v0.7.5 的 $\sim$5 min/ckpt 提升到 v0.7.14 5M-aligned 的 $\sim$5-30 min/ckpt（SLT 慢于其他，因为 8 层 ALIF 循环）。
