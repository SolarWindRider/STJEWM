...
\textbf{参数量注（v0.7.14 5M-aligned 之前）}：原始 STJEWM 的 6 个 readout 共享同一骨架（仅 readout 不同），trainable=2.70M，total=8.20M（含冻结的 ViT-Tiny 编码器 5.50M）；SNN/Transformer 基线各自独立骨架（未对齐到 5M）；MLP 基线是"坍缩对照"。这个设置下，trace dynamics 的校准性 vs MLP 坍缩的对比已经被观察到，但\textbf{参数规模不公平（STJEWM trainable=2.70M vs 其他基线 0.22-7.42M，33.7× 差距）}。
...
\begin{itemize}[leftmargin=2em]
\item 把 state\_projector 换成 \textbf{5.5M 冻结的 ViT-Tiny 像素编码器}（192-dim 输出，image\_size 84）外加 0.07M 可训练投影
