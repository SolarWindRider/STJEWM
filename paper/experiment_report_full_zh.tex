...
\midrule
\multirow{4}{*}{SNN / 神经形态对照} & \texttt{cubifae\_baseline} & ALIF + time-cell readout，$v_t$ 可读 & \textbf{4.98M} \\
    & \texttt{spikedreamer\_baseline} & LIF 编码 + AdaLN-zero Transformer & \textbf{5.12M} \\
    & \texttt{slt\_lif\_mpc\_trace} & 堆叠 LIF（自研），trace-only readout & \textbf{5.11M} \\
    & \texttt{slt\_lif\_mpc\_free} & 堆叠 LIF（自研），free-access readout & \textbf{5.05M} \\
\midrule
...
\textbf{重要声明}：以下三个 SNN 对照为\textbf{本项目自研实现}，名字是\textbf{架构功能描述}（不引用任何外部文献或代码库；早期代码注释中的文献名未经核实，已撤销）。代码与 checkpoint 目录中的历史标识（\texttt{cubifae\_baseline}、\texttt{spikedreamer\_baseline}、\texttt{slt\_lif\_mpc\_trace/free}）与功能名对应关系如下。它们复用 ST-JEWM 的 LIFCell 与投影组件构建。

\begin{itemize}[leftmargin=2em]
    \item \textbf{ALIF-timecell}（代码标识 \texttt{cubifae\_baseline}；\textbf{SNN}，自研，5M-aligned trainable = \textbf{4.98M}，\textbf{违反协议}——暴露膜电位）：\textbf{多时间尺度自适应 LIF（ALIF）}。每层 ALIF 维护 $v_t$，发放后 refractory 强制重置；$b$ 自适应阈值随脉冲累积增加；多层膜电位拼接后经 1D 卷积 time-cell 读出（kernel 256 / stride 128 → 8 个时间锚点）。\textbf{设计目的}：用“自适应阈值 + 显式时间卷积”替代 ST-JEWM 的 gated trace 做时间聚合，检验\textbf{时间聚合机制}是否决定校准（轴 1 对照）。规划器可读膜电位 $v_t$。5M 配置：\texttt{d\_hid=186, n\_layers=2}
    \item \textbf{LIF-Transformer}（代码标识 \texttt{spikedreamer\_baseline}；\textbf{混合 LIF + Transformer}，自研，5M-aligned trainable = \textbf{5.12M}，\textbf{违反协议}——读连续 Transformer 隐状态）：LIF 脉冲编码器\textbf{只做感知}，循环预测由 AdaLN-zero Transformer 承担。\textbf{设计目的}：检验“事件驱动\textbf{仅在感知层}、记忆为连续”的中间形态是否校准（轴 1 + 轴 2 对照；与 Stacked-LIF 的纯脉冲记忆直接对置）。规划器可读 Transformer 隐状态。5M 配置：\texttt{d\_snn=288, d\_tx=288, num\_layers=3, num\_heads=8}
    \item \textbf{Stacked-LIF-trace}（代码标识 \texttt{slt\_lif\_mpc\_trace}；\textbf{SNN}，自研，5M-aligned trainable = \textbf{5.11M}，\textbf{膜电位禁止}——协议合规）：堆叠 LIF 循环动力学，读出 $z = \text{moving\_average}(s_t, k{=}4)$（脉冲移动平均）。\textbf{设计目的}：最接近 ST-JEWM 的协议内对照——纯脉冲循环记忆 + 脉冲读出，区别仅在“移动平均”与“gated trace”两种时间滤波（轴 3 的协议内一极）。5M 配置：\texttt{d\_in=672, embed\_dim=672, n\_layers=8, trace\_beta=0.9, k\_avg=4}
    \item \textbf{Stacked-LIF-free}（代码标识 \texttt{slt\_lif\_mpc\_free}；\textbf{SNN}，自研，5M-aligned trainable = \textbf{5.05M}，\textbf{违反协议}——暴露膜电位）：同架构，但读出 $z = \text{concat}(s_t, v_t)$（脉冲 + 膜电位拼接）。\textbf{设计目的}：同一事件驱动循环动力学的\textbf{膜电位暴露孪生体}——与 trace 变体逐字对比，检验\textbf{读出接口是否是校准变量}（轴 3 的协议外一极）。5M 配置：\texttt{d\_in=640, embed\_dim=640, n\_layers=8}
...
\textbf{不同 baseline 的 native 损失}：ALIF-timecell 用 \texttt{cubifae\_loss}（pred + L1 spike sparsity）；LIF-Transformer 用 \texttt{spikedreamer\_loss}（pred + KL + recon + sparse）；Stacked-LIF 用 \texttt{slt\_lif\_mpc\_loss}（pred + sparse + action）；其他基线（LeWM Transformer / GRU / MLP）沿用 STJEWM 的 3 项损失。所有损失通过 \texttt{code/native\_losses.NATIVE\_LOSS\_DISPATCH} 在 \texttt{train.py:219} 处按 \texttt{args.model} 分派。
...
\textbf{为什么 STJEWM $\approx$ chance 而 Stacked-LIF-trace 0.67}：SLT 的 readout 是 \texttt{moving\_avg → Linear}（线性），把 trace 序列\textbf{线性可解码地}映射到事件类别——线性 probe 立刻能 recover 出来。STJEWM 的 gated trace 是\textbf{非线性门控}（$\alpha_t = \sigma(W[r_{t-1}, s_t, c_t])$），事件信息在 latent 里（(b) 中 $\rho \geq 0.99$ 表明脉冲与 obs 同步），但不在\textbf{线性可分}子空间。\textbf{这是 readout 设计的差异}，不是 trace 的失败。P13（3-epoch 重测）显示 LeWM AUROC 稳定在 0.63（probe-build artifact 修正后），STJEWM 在 1-epoch 的 0.50 是真实低值。
...
\textbf{实际结果（pixel）}：
\begin{itemize}[leftmargin=2em]
\item \textbf{三簇分界在极值上保留}：env-SR 上 LeWM-v2 cartpole 0.0--0.2（最低，过反应放大 obs 噪声致控制失败）；STJEWM/SLT 0.4--0.8（部分控制）；坍缩簇（MLP/LIF-Transformer）在静态可达 env 上偶尔得分高但 env-SR 区分度被静态目标稀释
