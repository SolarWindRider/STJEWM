...
\multirow{4}{*}{SNN / 神经形态对照} & \texttt{cubifae\_baseline} & ALIF + time-cell readout，$v_t$ 可读 & \textbf{4.98M} \\
    ...
    & \texttt{slt\_lif\_mpc\_trace} & DECOLLE LIF，trace-only readout & \textbf{5.11M} \\
    & \texttt{slt\_lif\_mpc\_free} & DECOLLE LIF，free-access readout & \textbf{5.05M} \\
\midrule
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{CuBiFAE baseline}（\textbf{SNN}，5M-aligned trainable = \textbf{4.98M}，违反协议）：多时间尺度 ALIF（Adaptive Leaky Integrate-and-Fire）SNN。每层 ALIF 维护 $v_t$，发放后 refractory 强制重置；ALIF 的 $b$ 自适应阈值随脉冲累积而增加。规划器可读 ALIF 的膜电位 $v_t$。5M 配置：\texttt{d\_hid=186, n\_layers=2}
    \item \textbf{SpikeDreamer baseline}（\textbf{混合 LIF + Transformer}，5M-aligned trainable = \textbf{5.12M}，违反协议）：LIF 脉冲编码后接 AdaLN-zero Transformer hidden state。规划器可读 Transformer 隐状态。5M 配置：\texttt{d\_snn=288, d\_tx=288, num\_layers=3, num\_heads=8}
    \item \textbf{SLT-LIF-MPC-trace}（\textbf{SNN}，5M-aligned trainable = \textbf{5.11M}，膜电位禁止）：DECOLLE 算法的 LIF 实现，3 阶段局部损失。读出 $z = \text{moving\_average}(s_t)$。膜电位禁止接口。5M 配置：\texttt{d\_in=672, embed\_dim=672, n\_layers=8, trace\_beta=0.9, k\_avg=4}
    \item \textbf{SLT-LIF-MPC-free}（\textbf{SNN}，5M-aligned trainable = \textbf{5.05M}，违反协议）：同架构，但读出 $z = \text{concat}(s_t, v_t)$，允许规划器读膜电位。5M 配置：\texttt{d\_in=640, embed\_dim=640, n\_layers=8}
