...
\textbf{关键结论}：三分法在 4.97M $\to$ 5.13M 范围内依然成立。STJEWM 6 readouts 都保持 \textbf{resp $\approx$ 0.21}、\textbf{div $\approx$ 0.01--0.03}（校准），与 \textbf{CuBiFAE} 和 \textbf{SLT-LIF-MPC} 处于同一 collapse-robust 簇。\textbf{MLP} 和 \textbf{SpikeDreamer} 是坍缩对照（resp$\to$0, div$\to$0, cos\_dist$\approx$0），\textbf{GRU} 和 \textbf{LeWM-v2} 是过反应（resp $\gg$ 1, div $\gg$ 0.05）。这验证了 trace dynamics 假设 \textbf{对参数规模鲁棒}：从 5.00M（MLP）到 5.13M（GRU）的 5M-aligned 范围内都呈现相同的 collapse-robust 区分。
...
\textbf{重要声明}：其中三个 SNN 对照（CuBiFAE / SpikeDreamer / SLT-LIF-MPC）为\textbf{本项目自研实现}，对应三个常见架构家族：自适应 LIF + time-cell 读出、LIF 编码器 + Transformer 预测器、堆叠 LIF（移动平均或膜电位暴露读出）。它们复用 ST-JEWM 的 LIFCell 与投影组件构建，\textbf{不依赖任何外部代码库或外部文献}；早期代码注释中标注的文献引用未经核实，已撤销。文中保留这些名字仅作为自研对照的标识，不构成对任何外部系统的引用。

\begin{itemize}[leftmargin=2em]
    \item \textbf{CuBiFAE baseline}（\textbf{SNN}，自研，5M-aligned trainable = \textbf{4.98M}，违反协议）：架构为多时间尺度 ALIF（Adaptive Leaky Integrate-and-Fire）。每层 ALIF 维护 $v_t$，发放后 refractory 强制重置；ALIF 的 $b$ 自适应阈值随脉冲累积而增加；多层膜电位拼接后经 1D 卷积 time-cell 读出。规划器可读 ALIF 的膜电位 $v_t$。5M 配置：\texttt{d\_hid=186, n\_layers=2}
    \item \textbf{SpikeDreamer baseline}（\textbf{混合 LIF + Transformer}，自研，5M-aligned trainable = \textbf{5.12M}，违反协议）：架构为 LIF 脉冲编码后接 AdaLN-zero Transformer hidden state。规划器可读 Transformer 隐状态。5M 配置：\texttt{d\_snn=288, d\_tx=288, num\_layers=3, num\_heads=8}
    \item \textbf{SLT-LIF-MPC-trace}（\textbf{SNN}，自研，5M-aligned trainable = \textbf{5.11M}，膜电位禁止）：架构为堆叠 LIF 预测模型。读出 $z = \text{moving\_average}(s_t)$。膜电位禁止接口。5M 配置：\texttt{d\_in=672, embed\_dim=672, n\_layers=8, trace\_beta=0.9, k\_avg=4}
    \item \textbf{SLT-LIF-MPC-free}（\textbf{SNN}，自研，5M-aligned trainable = \textbf{5.05M}，违反协议）：同架构，但读出 $z = \text{concat}(s_t, v_t)$，允许规划器读膜电位。5M 配置：\texttt{d\_in=640, embed\_dim=640, n\_layers=8}
    \item \textbf{LeWM Transformer baseline}（\textbf{Transformer}，5M-aligned trainable = \textbf{4.97M}，违反协议）：LeWM paper 的 AdaLN-zero Transformer（LeWM 原论文 6 层 5.07M；5M 配置 \texttt{embed\_dim=288, num\_layers=3, num\_heads=8}）。规划器可读 Transformer hidden state
    \item \textbf{GRU baseline}（\textbf{连续循环神经网络}，5M-aligned trainable = \textbf{5.13M}，违反协议）：\texttt{hidden=560, num\_layers=2}。标准 GRU 隐状态可读
    \item \textbf{MLP baseline}（\textbf{无状态 FFN}，5M-aligned trainable = \textbf{5.00M}，作为坍缩对照）：\texttt{hidden=640, num\_layers=12} 纯前馈，\textbf{无循环}。所有时间步都从当前 obs 重新预测。预期表现为 div $\to 0$ 的"坍缩控制"
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{我们做的对比}：下载官方代码，在 proprioceptive（状态输入）配置下于 4 个 DMC 任务（cartpole\_swingup、cheetah\_run、walker\_walk、finger\_spin）各训练 $5\times10^5$ 环境步（其 proprio 配置的公开预算），报告官方 episode return + 我们的协议指标（event-$\rho$、cos-dist、脉冲激活率）。
\end{itemize}
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{DMC 容差}：DMC \texttt{check\_success} 使用 \texttt{dist = $\|$\text{state} - \text{goal}$\|_2 / \sqrt{n_q}$} 与阈值 $\text{tol}=0.1$。对 9 个高维 locomotion env（\texttt{cheetah, walker, hopper, quadruped, humanoid, humanoid\_cmu, dog, fish, stacker}）的 87 维 state，随机 uniform 距离 $\approx 0.45 < 0.1$ 已不成立（随机通过率 0\%）。CEM 5 步规划器达不到 25 步目标，因此这些 env 的 env-SR 落在 0，\texttt{mean\_cos\_dist} 显露出真信号：STJEWM 6 个 readout 全部在 $[0.103, 0.123]$（v0.7.18.4 公平重跑），MLP/GRU/SpikeDreamer 接近 0（坍缩），LeWM-v2 在 0.183（过反应）
...
SpikeDreamer & 小 & 小 & $-0.0003$ & \textbf{坍缩} \\
