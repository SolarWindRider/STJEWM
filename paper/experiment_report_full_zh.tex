...
DMC（DeepMind Control）Suite 的 14 个 proprioceptive 任务：
...
\midrule
cartpole\_2d        & 5 × 1   & 经典控制 & 25  \\
pendulum\_2d        & 4 × 1   & 经典控制 & 25  \\
finger              & 3 × 1   & 经典控制 & 25  \\
ball\_in\_cup        & 6 × 2   & 经典控制 & 25  \\
reacher (4D)         & 7 × 2   & 稀疏 POMDP & 25  \\
cheetah             & 6 × 6   & 运动     & 25  \\
walker              & 8 × 6   & 运动     & 25  \\
hopper              & 8 × 3   & 运动     & 25  \\
quadruped           & 12 × 6  & 运动     & 25  \\
humanoid            & 28 × 6  & 运动     & 25  \\
humanoid\_CMU       & 63 × 6  & 运动     & 25  \\
dog                 & 79 × 6  & 运动     & 25  \\
fish                & 13 × 5  & 运动     & 25  \\
stacker              & 11 × 6  & 运动     & 25  \\
\bottomrule
...
\textbf{什么是 obs\_dim 和 action\_dim}：\\
\begin{itemize}[leftmargin=2em]
    \item \textbf{obs\_dim}：观测的维度。例如 cartpole\_2d（小车摆杆）有 5 个数：位置、角度、角速度等
    \item \textbf{action\_dim}：动作的维度。cartpole\_2d 只能左右推（1 维）；humanoid 有 28 个关节（28 维）
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{F1 PushT}（LeWM-style 2D 块推动）：\textbf{什么是 PushT}：一个滑块需要被推到目标位置。obs = 5 维（目标 xy、滑块 xy、滑块角度），goal\_offset = 100 帧
    \item \textbf{F2 TwoRoom}（LeWM 部分观测导航）：\textbf{什么是 TwoRoom}：智能体在一个 2D 平面上只有部分观测，需要推断目标位置。obs = 4 维，goal\_offset = 100 帧
    \item \textbf{F3 Reacher}（4D reacher）：\textbf{什么是 Reacher}：2 关节机械臂需要把末端执行器移到目标点。obs = 7 维，goal\_offset = 25 帧
    \item \textbf{F4 DMC}（14 个 DMC 环境 held out）：见上表
\end{itemize}

\textbf{训练集 vs 评测集}：跨基准族训练使用 15-16 个环境的并集（gating family held out），评测在 held-out family 上。例如，PushT-held-out 的训练集是 \{DMC × 14, Reacher, TwoRoom\}（15 个环境），评测只在 PushT 上。
...
所有 STJEWM 模型共享\textbf{同一个 8.20M 参数的纯 SNN 骨架}（脉冲神经元堆叠 4 层，embed\_dim = 192）。区别\textbf{仅在最后一步}——规划器看到什么：

\begin{itemize}[leftmargin=2em]
    \item \textbf{trace\_only}（默认，膜电位禁止）：$z_\text{final} = r_t$ = 脉冲后的轨迹。规划器只能看到有界的、内容感知的轨迹
...
\textbf{数学上}，trace dynamics 是：

\[
r_t = \alpha_t \cdot r_{t-1} + (1 - \alpha_t) \cdot s_t, \quad \alpha_t = \sigma(W[r_{t-1}, s_t, c_t])
\]

即：新的轨迹是"老的轨迹 × 遗忘门 + 当前脉冲 × (1 - 遗忘门)"。当没有脉冲时，$r$ 衰减；当有脉冲时，$r$ 跳到接近 1。遗忘门由过去 + 现在 + 上下文决定，所以这个动力学\textbf{本身就是内容感知的低通滤波器}。

\subsection{6 个非 STJEWM 对比模型}

\begin{itemize}[leftmargin=2em]
    \item \textbf{CuBiFAE baseline}（\textbf{SNN}，7.5M 参数，违反协议）：\textbf{什么是 CuBiFAE}：多时间尺度 ALIF（Adaptive Leaky Integrate-and-Fire，脉冲神经元的标准模型）SNN。规划器可读 ALIF 的膜电位 $v_t$。
    ...
    \item \textbf{LeWM Transformer baseline}（\textbf{Transformer}，5.07M 参数，违反协议）：纯 Transformer 世界模型。规划器可读 Transformer hidden state。
...
\begin{itemize}[leftmargin=2em]
    ...
    \item 批次大小：32
    \item 训练轮数：1 epoch（within-DMC）或 5 epochs（跨基准族）
    \item 网络结构：embed\_dim = 192, n\_layers = 2, n\_d = 3（MultiComp compartment 数量）
\end{itemize}

\textbf{解释}：
\begin{itemize}[leftmargin=2em]
    \item \textbf{epoch}：遍历整个训练集 1 次
    \item \textbf{lr}：学习率——优化器每步更新权重的步长
    \item \textbf{weight\_decay}：L2 正则化权重——防止权重过大
    \item \textbf{$\lambda_\text{sigreg}$}：信号正则化损失权重——鼓励脉冲稀疏
    \item \textbf{$\lambda_\text{goal}$}：goal-prediction 损失权重——核心预测任务的损失权重
\end{itemize}
...
\begin{longtable}{p{0.25\textwidth}p{0.30\textwidth}p{0.20\textwidth}p{0.18\textwidth}}
\toprule
\textbf{评测轴} & \textbf{splits × models × envs} & \textbf{总计 cells} & \textbf{目的} \\
\midrule
DMC 内部子族 (轴 1) & 6 × 12 × 14 DMC envs   & 1008 & gating experiment \\
跨基准族 (轴 2) & 4 × 12 × (1 或 13) envs & 192 & 边界条件 \\
\textbf{总计} & & \textbf{1200} & \\
\bottomrule
\end{longtable}
...
在重新评测跨基准族之前，我们发现并修复了 2 个严重的评测 bug：

\begin{enumerate}[leftmargin=2em]
    \item \textbf{Bug \#1 (DMC 阈值过松)}：原代码 \texttt{check\_success} 的容差 = 1.0，\textbf{随机 uniform 状态通过率 87-100\%}。这是 DMC 内部子族实验里"SNN 全部 env-SR = 100\%"的假象来源。修复后 \texttt{tol = 0.1}，随机通过率 0\%。
    \item \textbf{Bug \#2 (LeWM@0.1 阈值被常数 latent 平凡通过)}：原始 \texttt{LeWM-SR} 阈值 0.1 让 MLP（输出常数零）也能达到 100\%。修复后改为 \texttt{LeWM@0.05}（更严），并以原始 \texttt{mean\_cos\_dist} 为主指标。
\end{enumerate}

\textbf{修复后的影响}：DMC 内部子族实验的 env-SR 全部从 1.0 降到 0（这是事实，CEM 5 步规划器达不到 25 步目标），但 \texttt{mean\_cos\_dist} 显露出真信号：STJEWM family 6 个 readout 全部在 [0.094, 0.116]，而 MLP/GRU 接近 0（坍缩），LeWM-v2 在 0.17-0.19（过反应）。
