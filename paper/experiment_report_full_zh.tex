...
calibrated: Stacked-LIF-trace & 0.115 $\pm$ 0.004 & [0.104, 0.125] & $-8.5$ \\
...
\textbf{3-seed verdict}：calibrated 簇内 pairwise CI 重叠（STJEWM-trace 与 Stacked-LIF-trace、STJEWM-spike 不可区分）；calibrated vs over-react 与 calibrated vs collapse 的 CI disjoint（Cohen's d 分别 $-7$--$-8.5$ 和 $>20$）。三簇在 3-seed 分辨率下\textbf{统计独立}。
...
\item \textbf{cheetah}：Stacked-LIF-trace 0.40 vs LeWM-v2 0.00 vs STJEWM 0.60（局部）
\item \textbf{pendulum}：Stacked-LIF-free 最高 1.00，LeWM-v2 0.00
...
\midrule
calibrated (SNN) & 0.171--0.178 & STJEWM-trace 0.171, Stacked-LIF-trace 0.178 \\
over-react & \textbf{0.091} (lowest) & LeWM-v2——过反应放大 obs 噪声致控制失败 \\
collapse (state) & n/a (静态可达) & MLP 0.172, LIF-Transformer/Stacked-LIF-free 接近 \dots \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\item \textbf{像素 env-SR（静态 goal, 13 模型均值 across 10 splits）}：STJEWM-trace 0.171, Stacked-LIF-trace 0.178, Stacked-LIF-free 接近, LeWM-v2 0.091（lowest）, MLP 0.172——三簇保留但 cos\_dist 尺度被 ViT 14×14 patch 放大（pixel 0.6--0.9 vs state 0.05--0.3）
