...
\noindent\textbf{v0.7.10b headline result (read first).} This release
adds the \textbf{v0.7.10b OOD Path-C} (3 DMC sub-families, 6 splits,
12 model variants, 39 held-out envs = 468 cells), which is \emph{the
gating experiment for the working title}. The headline number is
STJEWM $\rho \in [0.9676, 0.9986]$ in every split, while non-SNN
baselines each fail at a distinct axis. The paper's structure has
been restructured around this OOD finding: \S7 (Cross-Environment
Transfer) now leads with the OOD Path-C matrix in \S7.6 and demotes
the v0.7.8 within-suite pilot to a single subsection. The 4
collapse-robust metrics (divergence, responsiveness, event-alignment
$\rho$, env-SR) are introduced in \S6 and re-applied as the
\emph{only} axes that discriminate the families. The working title
``generalisable world models'' is now supported \emph{within DMC
sub-families}. The cross-benchmark-family axis (Pusht / LeWM reacher
/ Tworoom / Delayed POMDP) and the cross-modality axis (state $\to$
pixel) are deferred to a future paper that requires a raw-obs
branch in STJEWM.
...
\subsection{7. Cross-Sub-Family Transfer: the v0.7.10b OOD Path-C (the gating experiment)}

This section is the gating experiment for the working title. \S7.1--\S7.5
review the historical v0.7.8 within-suite leave-two-envs-out pilot
(4 of 12 ckpts retrained, within the G16 heterogeneous suite).
\textbf{\S7.6 is the new v0.7.10b OOD Path-C} (3 DMC sub-families,
6 splits, 12 model variants, 39 held-out envs = 468 cells), which
is the main contribution of this release and \emph{supports} the
working title ``generalisable world models'' within DMC sub-families.

\subsubsection{7.0 Honest scope statement (read first)}
...
\item \emph{The honest take-away from this within-suite pilot}: the
calibrated regime is \emph{largely preserved} on the held-out envs.
We use the language \emph{largely preserved} / \emph{shows limited
drift} / \emph{in the same diagnostic regime}, never
\emph{invariant}. The within-suite transfer pilot is not the
gating experiment for the working title.
\end{itemize}
...
\subsection{Trace-friendly task negative result (delayed\_tMaze, v0.7.10b)}

We ran a targeted probe to test whether the gated exponential trace
readout (STJEWM \texttt{trace\_only}) outperforms the membrane
readout (STJEWM \texttt{membrane\_readout}) and the multi-timescale
passive decay readout (CuBiFAE) on a deliberately event-aligned,
sparse-cue task: \texttt{delayed\_t\_maze} (state 6D, action 2D,
3-frame cue phase then 7- or 47-frame pure-forward corridor before
a binary left/right choice).
...
the same decoding bottleneck that \S9.1 (latent-goal MPC) and
\S6 (env-SR saturates) already established; this targeted probe
...
target state without going through CEM --- but such a controller is
no longer testing the \emph{predictive-state} question. We do not
have such a result, and we do not claim one. The full per-cell JSONs
are at \texttt{results/generalist\_G15\_trace\_demo/eval/} and the
summary table is
...
To test the content-aware-rate-counter hypothesis from \S9.5 directly,
...
current window. The action is \emph{purely observational} (it does
not influence the env's event stream), so the \emph{only} signal
the model has for the modal event is its \textbf{integrated
content-aware trace} of the recent events.
...
both win over CuBiFAE on this content-aware task by $\sim$2
percentage points, direction-consistent across all 3 seeds. Trace
...
rate counter}: it integrates the recent event stream and detects
the modal event with 20\% accuracy on a 5-class task with 30\%
...
bottleneck named in \S9.5: the env draws events independently of
...
ties the trace readout (\S6, \S7, \S9.1, \S9.5, \S9.6), the
...
\subsection{Cross-benchmark family OOD (v0.7.12, partial negative result)}

The cross-benchmark-family axis is the \emph{true} OOD axis
(different benchmark families, not just different sub-families
of DMC). We ran 3 splits (one family held out at a time, from the
...
F3 (Reacher) & stjewm\_trace\_only       & reacher & 0.578 & 0.033 \\
F3 (Reacher) & stjewm\_membrane\_readout & reacher & 0.556 & 0.033 \\
F4 (DMC held out) & cubifae\_baseline        & 13 DMC envs (avg) & 0.506 & --- \\
F4 (DMC held out) &stjewm\_trace\_only       & 13 DMC envs (avg) & 0.506 & --- \\
F4 (DMC held out) &stjewm\_membrane\_readout & 13 DMC envs (avg) & 0.518 & --- \\
\bottomrule
\end{tabular}
\caption{Cross-benchmark family OOD result. STJEWM does not have a
universal hard performance win on cross-benchmark-family transfer.
The result is split: STJEWM (membrane readout) wins clearly on F1
(+24.4 pp on LeWM-SR); CuBiFAE wins on F2 (+10 pp); all three tied
within noise on F3.}
\label{tab:cross-benchmark}
\end{table}

\textbf{Result: STJEWM does not have a universal hard performance
win on cross-benchmark-family OOD.} The result is split: STJEWM
(membrane readout) wins clearly on F1 / PushT (+24.4 pp on
LeWM-SR); CuBiFAE wins on F2 / TwoRoom (+10 pp); all three tied
within noise on F3 / Reacher. STJEWM does not generalise uniformly
across benchmark families.

\textbf{Implication for the working title.} The working title
``generalisable world models'' is \textbf{supported within DMC
sub-families} (v0.7.10b, \S7.6) but \textbf{not supported across
benchmark families} (v0.7.12, this section). The honest scope
is that STJEWM is a \emph{methodological contribution} (a probe
for the membrane-forbidden protocol), not a
\emph{universal-performance contribution} on cross-benchmark-family
transfer. Full per-cell JSONs at
\texttt{results/cross\_benchmark\_F\{1,2,3\}/eval/} and summary at
\texttt{results/cross\_benchmark\_F1/eval/RESULTS.md}.
