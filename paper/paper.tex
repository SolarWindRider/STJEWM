% ST-JEWM v0.7.9 - Learning Calibrated Predictive States for Generalizable World Models
...
\noindent\textbf{v0.7.9 honest-scope correction (read first).} The v0.7.8
release described §7 as ``cross-environment generalisation'' and called the
within-suite leave-two-env-out pilot sufficient evidence for the working
title ``Generalisable World Models''. On reviewer feedback the framing has
been downgraded: §7 is now ``Cross-Environment Transfer and Scaling
(Within Suite)''; OOD1/OOD2/OOD3 (14-split cross-benchmark-family matrix)
is the gating experiment for the working title and is deferred to v0.7.10;
§7 uses \emph{largely preserved} / \emph{shows limited drift} / \emph{in the
same diagnostic regime}, never \emph{invariant}; §9 downgrades the
``only-family-generalises'' claim to ``only-family-among-the-4-retrained''.
All numbers in the tables are unchanged; only the interpretation is.
