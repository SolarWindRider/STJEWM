...
\begin{quote}\small
\texttt{obs}, $a_t$ $\longrightarrow$ MultiCompStack SNN (4 layers, embed-dim~192)\\
\hspace*{4em}$\downarrow$\\
\hspace*{4em}$v_t = \Phi(v_{t-1}, x_t, a_{t-1})$ \hfill (internal only)\\
\hspace*{4em}$s_t = \mathbb{1}[v_t > \vartheta]$ \hfill spike (event)\\
\hspace*{4em}$r_t = \alpha_t \odot r_{t-1} + (1-\alpha_t) \odot s_t$ \hfill trace\\
\hspace*{4em}$\downarrow$\\[2pt]
\hspace*{4em}Predictor / CEM planner: \textbf{input} $\equiv r_t$ (bounded, event-driven)\\
\hspace*{4em}\hspace*{4em}$v_t$ \quad $\times$ \textbf{FORBIDDEN}\\
\hspace*{4em}loss $= d(\hat{g}_\theta(r_t, a_t),\, \mathrm{sg}(E(o_{t+1})))$
\end{quote}
...
the planner (\emph{membrane-readout}, \S 3.5), or route a Transformer hidden
representation next to the trace (\emph{hidden-leak}), are explicitly out
...
\begin{quote}\small
\begin{tabular}{ll}
observation event-strength $\|\Delta o_t\|$ & (orange) \\
latent-difference $\|\Delta z_t\|$ & (blue) \\
STJEWM-trace spike train $s_t$ & (gray ticks) \\
\end{tabular}\\[4pt]
\begin{tabular}{ll}
STJEWM-trace: & $\rho = 0.840$ on \texttt{cheetah} (latent follows obs events) \\
LeWM Transformer: & $\rho = 0.606$ on \texttt{cheetah} (latent lags, drifts, 30$\times$ amplification) \\
GRU: & $\rho = 0.057$ on \texttt{cheetah} (flat, decoupled) \\
MLP collapse-control: & $\rho = -0.031$ (constant latent by construction) \\
\end{tabular}
\end{quote}

Two DMC environments (\texttt{cheetah}, \texttt{finger}) are visualised
across one 500-step random-policy trajectory. For each: (a) per-step
observation first-difference $\|\Delta o_t\|$; (b) per-step latent
first-difference $\|\Delta z_t\|$ from the predictive latent; (c) STJEWM
spike train for the trace readout. STJEWM-trace aligns latent-change
peaks with observation-event peaks ($\rho \approx 0.84$ on
\texttt{cheetah}). LeWM Transformer has a high-mean, low-correlation
latent that drifts even when the observation is stationary
($\rho = 0.61$, latent amplification $\sim 30\times$). GRU is
flat-on-purpose ($\mathrm{resp}\,30\times$, $\rho = 0.06$). MLP
collapse-control is literally flat (constant latent by definition,
$\rho = -0.03$). Per-time traces for the second half (\texttt{finger},
\texttt{ball\_in\_cup}, \texttt{walker}, etc.) are in Supplementary
Appendix E.
...
\hline
 ...
 & std(20) & stress(4) & std(20) & stress(4) & probe-AU & align-$\rho$ \\
\hline
STJEWM trace-only       & 67.1 & 25.0 & 73.5 & \textbf{66.5} & 0.690 & 0.626 \\
STJEWM spike-only       & 65.9 & 25.0 & 66.5 & 57.5 & \textbf{0.699} & 0.621 \\
...
CuBiFAE                 & \textbf{69.5}\textsuperscript{*} & 25.5 & 76.3 & 52.5 & 0.569 & 0.638 \\
SpikeDreamer            & 68.3 & \textbf{41.5} & n/a\textsuperscript{*} & n/a\textsuperscript{*} & 0.474 & n/a \\
SLT-LIF-MPC trace       & 68.6 & 25.0 & 72.6 & 47.5 & 0.533 & 0.636 \\
SLT-LIF-MPC free        & 65.7 & 26.5 & 66.7 & 66.5 & 0.504 & \textbf{0.640} \\
\hline
LeWM Transformer        & 68.2 & 25.5 & 76.9 & 56.5 & 0.166 & 0.160 \\
GRU                     & 66.6 & \textbf{42.0} & \textbf{78.8} & 51.0 & 0.574 & $-0.011$ \\
MLP (collapse-ctrl)     & 64.7 & 32.5 & \textbf{98.0}\textsuperscript{\dag} & \textbf{95.5}\textsuperscript{\dag} & 0.524 & $-0.002$ \\
\hline
\end{tabular}\\[4pt]
{*best non-STJEWM \hspace{2em} \dag MLP LeWM-SR is a collapse artefact; see §2.3, §6.3}
\end{quote}
...
\\[6pt]
Four clusters separated on the G16 scatter (LeWM-SR vs divergence):
\begin{itemize}
\item \textbf{collapse} (upper-left, MLP): LeWM-SR $95.5$\%, div $0.0002$;
constant latent by construction.
\item \textbf{noise} (right of MLP, GRU): div $0.007$, event-align
$\rho \approx -0.07$; latent moves randomly with each input.
\item \textbf{over-reactive} (right side, LeWM-v2): div $\sim 16\times$
calibrated, event-align $\rho = 0.52$; latent amplifies obs by
$\sim 30\times$ and feeds back into the planner as a Transformer hidden
state.
\item \textbf{calibrated} (lower-middle, STJEWM family + CuBiFAE +
SLT-LIF-MPC): div $0.011 \pm 0.001$, event-align $\rho \geq 0.99$.
\end{itemize}
\end{quote}
...
\begin{quote}\small
\textbf{Per-dim std of latent trajectory} (200 random-policy steps $\times$ 6 envs).
STJEWM family + CuBiFAE + SLT-LIF-MPC stay in the calibrated band
($0.011 \pm 0.001$) at G4, G8, and G16. MLP collapse-control is exactly
$0.0002$ at every scale --- the collapse signature is scale-invariant.
LeWM Transformer is $\sim 16\times$ calibrated (over-reactive). GRU
is normal on div (noise regime).
\\[4pt]
\textbf{Responsiveness} $\mathrm{mean}(\|\Delta z\|)/\mathrm{mean}(\|\Delta o\|)$.
STJEWM family $\approx 0.21$ at every scale. GRU $\approx 30$ (resp
$150\times$ STJEWM); LeWM-v2 $\approx 30$ (resp $150\times$ STJEWM).
MLP $\approx 0.55$ --- collapsed and motionless.
\\[4pt]
\textbf{Event-alignment $\rho$}.
STJEWM family $\geq 0.99$ at every scale. LeWM-v2 $0.52$. GRU
$\rho \approx -0.07$. MLP $\rho \approx 0$ (constant latent).
\end{quote}

Figure 4 condenses §6.4--§6.6 onto a single visual. The key result is
\emph{cross-suite scale-invariance}: STJEWM family + CuBiFAE +
SLT-LIF-MPC stay in the calibrated band at G4 / G8 / G16; the
collapse signature of MLP ($\mathrm{div} = 0.0002$) persists at every
scale; the over-reactivity of LeWM and the noise of GRU are stable.
This is what the protocol's central empirical claim rests on: the
trace dynamics produce calibrated latents that \emph{survive} the
shared-weight generalist regime, not just the specialist regime.
